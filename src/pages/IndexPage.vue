...
<script>
import { useProducts } from "stores/all";
export default {
  async preFetch() {
...
<script setup>
import { useTheme } from "stores/all";
...
onBeforeMount(() => {
  const { store: themeStore, indexScripts } = useTheme();
  themeStore.sidebar = true;
  indexScripts();
});

const { store: productStore } = useProducts();
</script>
